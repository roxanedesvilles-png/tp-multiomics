...
set.seed(1234)
# Filtrage
rna_keep_bis <- !is.na(cv_rna) & cv_rna >= 1.5
rna_filt_bis <- rna_mat[, rna_keep, drop = FALSE]

# Scaling
rna_scaled_bis <- scale(rna_filt)
dim(rna_scaled_bis)

mi  <- build.mim(rna_scaled_bis, estimator = "spearman")
clr_mat <- clr(mi)
...
gene_names <- colnames(rna_scaled_bis)

isolated_genes <- gene_names[rowSums(clr_mat) == 0]
...
hist(degree(g_grn), breaks = 50, main = "Distribution des degrés", xlab = "Degré", ylab = "Fréquence")

BioGRID <- readRDS("human_ppi.Rds")
str(BioGRID)
