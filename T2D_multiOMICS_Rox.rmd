...
network(final.diablo.model, blocks = c(1,2,3),
        color.node = c('darkorchid', 'brown1', 'lightgreen'), cutoff = 0.85)
```


# **7. Analyse en réseau**

```{r GRN-analysis, message=FALSE, warning=FALSE}
library(netOmics)
library(igraph)

# 1. Extraire les gènes sélectionnés par DIABLO
selected_genes <- selectVar(final.diablo.model, block = "RNA")$RNA$name

# 2. Filtrer la matrice RNA pour ne garder que ces gènes
rna_filtered <- rna_scaled[, selected_genes, drop = FALSE]

# 3. Construire le GRN avec corrélation
# cor.netOmics retourne une liste avec $adjacency
grn_result <- cor.netOmics(rna_filtered)

# 4. Convertir en objet igraph
grn_igraph <- graph_from_adjacency_matrix(
  grn_result$adjacency,
  mode = "undirected",
  weighted = TRUE,
  diag = FALSE
)
...
# 5. Analyse du réseau
cat("Nombre de nœuds :", gorder(grn_igraph), "\n")
cat("Nombre d'arêtes :", gsize(grn_igraph), "\n")
cat("Nombre de nœuds isolés :", sum(degree(grn_igraph) == 0), "\n")

# Distribution des degrés
degree_dist <- degree(grn_igraph)
hist(degree_dist, main = "Distribution des degrés", xlab = "Degré")

# Gène le plus connecté
most_connected_gene <- names(which.max(degree_dist))
cat("Gène le plus connecté :", most_connected_gene, "\n")

# 6. Visualisation basique
plot(
  grn_igraph,
  vertex.size = 5,
  vertex.label = NA,
  edge.width = E(grn_igraph)$weight * 2,
  main = "GRN - gènes filtrés par DIABLO"
)
```
